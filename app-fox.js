(() => {
  ...
  const wifiNetworkNames = ["Home_5G", "LUMIQ Studio", "Guest", "Office_WiFi", "LUMIQ_2G", "Studio_Guest", "Cafe_Free", "Visitor_WiFi"];
  const onboardingSteps = [
    {
      ...
      primary: "Begin setup",
      icon: "arrow-right",
      visual: '<div class="onboarding-visual" aria-hidden="true"></div>'
    },
    {
      id: "language",
      count: "Step 2 of 12",
  ...
  const friendsFlowAllSteps = [
    {
      id: "nearby", kicker: "FRIEND NEARBY", title: "A friend is close", subtitle: "Milo is nearby. Say hello only when you are ready.", visual: "nearby",
      primary: { label: "Confirm", icon: "check", target: "partners" }, secondary: { label: "Later", icon: "x", target: "apps" }
    },
    {
      id: "hello", kicker: "SAY HELLO", title: "Say hello to Milo?", subtitle: "Only your approved profile details will be shared.", visual: "hello",
      primary: { label: "Say hello", icon: "hand", target: "new-friend" }, secondary: { label: "Not now", icon: "x", target: "apps" }
    },
    {
      id: "new-friend", kicker: "NEW FRIEND", title: "You are friends", subtitle: "Milo was added to your private friends list.", visual: "success",
      primary: { label: "Send wave", icon: "send", target: "milo-hi" }, secondary: { label: "Done", icon: "check", target: "apps" }
    },
    {
      id: "milo-hi", kicker: "FRIEND ACTIVITY", title: "Milo says hi", subtitle: "A new friend motion is waiting for your reply.", visual: "wave",
      primary: { label: "Choose a bond", icon: "heart-handshake", target: "bond" }, secondary: { label: "Later", icon: "clock-3", target: "apps" }
    },
    {
      id: "bond", kicker: "A FRIEND OR PARTNER", title: "Choose a bond", subtitle: "Keep it light or build something together.", visual: "bond",
      options: [
        { label: "Make friends", icon: "hand-heart", action: "make-friends" },
        { label: "Become partners", icon: "sparkles", action: "become-partners" }
      ]
    },
    {
      id: "partner-request", kicker: "PARTNER REQUEST", title: "Partner with Milo?", subtitle: "Partnership is mutual and can be ended by either side.", visual: "partner",
      primary: { label: "Send request", icon: "send", target: "waiting" }, secondary: { label: "Not now", icon: "x", target: "apps" }
    },
    {
      id: "waiting", kicker: "WAITING FOR MILO", title: "Waiting for Milo", subtitle: "You can keep playing while Milo decides.", visual: "waiting",
      primary: { label: "Keep playing", icon: "play", target: "partners" }, secondary: { label: "Cancel", icon: "x", target: "apps" }
    },
    {
      id: "partners", kicker: "PARTNERS", title: "You are partners", subtitle: "Create a new Avatar from both companions.", visual: "partners",
      primary: { label: "Confirm", icon: "check", target: "home" }, secondary: { label: "Later", icon: "x", target: "apps" }
    },
    {
      id: "creating", kicker: "CREATING TOGETHER", title: "Mixing your Avatars", subtitle: "Combining approved traits into a new companion.", visual: "creating",
      primary: { label: "Continue", icon: "arrow-right", target: "avatar-born" }, secondary: { label: "Later", icon: "clock-3", target: "apps" }
    },
    {
      id: "avatar-born", kicker: "NEW AVATAR", title: "Made together", subtitle: "Your shared Avatar is ready to meet you.", visual: "born",
      primary: { label: "Meet Avatar", icon: "sparkles", target: "companion" }, secondary: { label: "Done", icon: "check", target: "apps" }
    },
    {
      id: "companion", kicker: "COMPANION", title: "Milo's Avatar", subtitle: "Choose a gentle interaction.", visual: "companion",
      options: [
        { label: "Feed", icon: "utensils", action: "feed" },
        { label: "Clean", icon: "sparkles", action: "clean" },
        { label: "Health", icon: "heart-pulse", action: "rest" },
        { label: "Explore", icon: "compass", action: "avatar" }
      ]
    },
    {
      id: "feed", kicker: "SNACK TIME", title: "A little snack", subtitle: "Tap once to feed your shared Avatar.", visual: "feed",
      primary: { label: "Feed", icon: "utensils", target: "companion" }, secondary: { label: "Back", icon: "arrow-left", target: "companion" }
    },
    {
      id: "clean", kicker: "CLEAN UP", title: "Fresh and tidy", subtitle: "A short clean-up keeps Milo comfortable.", visual: "clean",
      primary: { label: "Clean", icon: "sparkles", target: "companion" }, secondary: { label: "Back", icon: "arrow-left", target: "companion" }
    },
    {
      id: "rest", kicker: "REST & CARE", title: "Time to rest", subtitle: "Stay close and offer a quiet moment.", visual: "rest",
      primary: { label: "Comfort", icon: "heart", target: "companion" }, secondary: { label: "Back", icon: "arrow-left", target: "companion" }
    },
    {
      id: "avatar", kicker: "AVATAR VIEW", title: "Explore the Avatar", subtitle: "Drag up or down to inspect the shared traits.", visual: "avatar",
      primary: { label: "Universe view", icon: "orbit", target: "universe" }, secondary: { label: "Back", icon: "arrow-left", target: "companion" }
    },
    {
      id: "universe", kicker: "UNIVERSE VIEW", title: "A wider world", subtitle: "Explore the relationship around your Avatar.", visual: "universe",
      primary: { label: "Cell view", icon: "scan", target: "cell" }, secondary: { label: "Back", icon: "arrow-left", target: "avatar" }
    },
    {
      id: "cell", kicker: "CELL VIEW", title: "Details at a glance", subtitle: "Return to the Avatar view when you are ready.", visual: "cell",
      primary: { label: "Back to Avatar", icon: "arrow-up", target: "avatar" }, secondary: { label: "Done", icon: "check", target: "companion" }
    },
    {
      id: "paused", kicker: "CREATION PAUSED", title: "Nothing was lost", subtitle: "The shared Avatar is safe. Retry when both friends are ready.", visual: "paused",
      primary: { label: "Retry", icon: "refresh-cw", target: "creating" }, secondary: { label: "Later", icon: "clock-3", target: "apps" }
    }
  ];

  // Keep the full Friends flow ready for later, while this preview exposes only the reference states.
  const friendsVisibleStepIds = new Set(["nearby", "milo-hi", "partners"]);
  const friendsFlowSteps = friendsFlowAllSteps.filter((step) => friendsVisibleStepIds.has(step.id));
  ...
  function friendsFlowVisualMarkup(step) {
    const companion = getCompanion();
    const avatar = `<img class="friends-flow-avatar" src="${companion.choiceAsset}" alt="${companion.name}, your ${companion.species} companion">`;
    const secondAvatar = `<img class="friends-flow-avatar friends-flow-avatar-secondary" src="${companion.choiceAsset}" alt="Milo's shared Avatar">`;

    if (step.visual === "paused") {
      return `<div class="friends-flow-visual friends-flow-visual-paused"><span class="friends-flow-status-icon"><i data-lucide="circle-alert" aria-hidden="true"></i></span><strong>Creation paused</strong><small>Your shared Avatar is safe</small></div>`;
    }

    if (["universe", "cell"].includes(step.visual)) {
      return `<div class="friends-flow-visual friends-flow-visual-${step.visual}"><span class="friends-flow-pair">${avatar}${secondAvatar}</span><span class="friends-flow-visual-badge"><i data-lucide="${step.visual === "partners" ? "heart" : step.visual === "creating" ? "sparkles" : step.visual === "universe" ? "orbit" : "scan"}" aria-hidden="true"></i></span>${step.visual === "creating" ? '<span class="friends-flow-progress-track"><i></i></span>' : ""}</div>`;
    }

    if (step.visual === "bond") {
      return `<div class="friends-flow-visual friends-flow-visual-bond"><span class="friends-flow-bond-ring">${avatar}</span><span class="friends-flow-visual-badge"><i data-lucide="heart-handshake" aria-hidden="true"></i></span></div>`;
    }

    if (step.id === "nearby") {
      return `<div class="friends-flow-visual friends-flow-visual-nearby-greeting"><video class="friends-flow-greeting-video friends-flow-nearby-video" src="animal-greeting-nearby.mp4" autoplay muted loop playsinline preload="auto" aria-label="Animal greeting"></video><span class="friends-flow-visual-badge"><i data-lucide="hand" aria-hidden="true"></i></span></div>`;
    }

    if (step.id === "partners") {
      return `<div class="friends-flow-visual friends-flow-visual-greeting friends-flow-visual-partners-greeting"><video class="friends-flow-greeting-video" src="animal-greeting.mp4" autoplay muted loop playsinline preload="auto" aria-label="Animal greeting"></video><span class="friends-flow-visual-badge"><i data-lucide="sparkles" aria-hidden="true"></i></span></div>`;
    }

    if (step.id === "creating") {
      return `<div class="friends-flow-visual friends-flow-visual-greeting friends-flow-visual-creating-greeting"><video class="friends-flow-greeting-video" src="animal-greeting.mp4" autoplay muted loop playsinline preload="auto" aria-label="Animal greeting"></video><span class="friends-flow-visual-badge"><i data-lucide="sparkles" aria-hidden="true"></i></span></div>`;
    }

    if (step.id === "new-friend") {
      return `<div class="friends-flow-visual friends-flow-visual-success friends-flow-visual-greeting"><video class="friends-flow-greeting-video" src="animal-greeting.mp4" autoplay muted loop playsinline preload="auto" aria-label="Animal greeting"></video><span class="friends-flow-visual-badge"><i data-lucide="check" aria-hidden="true"></i></span></div>`;
    }

    const icon = step.visual === "success" || step.visual === "born" ? "check" : step.visual === "wave" ? "send" : step.visual === "waiting" ? "ellipsis" : step.visual === "companion" ? "sparkles" : step.visual === "feed" ? "utensils" : step.visual === "clean" ? "sparkles" : step.visual === "rest" ? "heart" : step.visual === "avatar" ? "move-vertical" : "hand";
    return `<div class="friends-flow-visual friends-flow-visual-${step.visual}"><span class="friends-flow-orbit" aria-hidden="true"></span><span class="friends-flow-character">${avatar}</span><span class="friends-flow-visual-badge"><i data-lucide="${icon}" aria-hidden="true"></i></span></div>`;
  }

  function friendsFlowActionMarkup(step) {
    if (step.options) {
      return `<div class="friends-flow-actions friends-flow-actions-grid">${step.options.map((option, index) => `<button class="friends-flow-action friends-flow-option${index === 0 ? " is-primary" : ""}" type="button" data-action="friends-flow-action" data-friends-action="${option.action}"><i data-lucide="${option.icon}" aria-hidden="true"></i><span>${option.label}</span></button>`).join("")}</div>`;
    }

    const actions = [
      step.secondary && { ...step.secondary, isPrimary: false },
      step.primary && { ...step.primary, isPrimary: true }
    ].filter(Boolean).map((action) => `<button class="friends-flow-action${action.isPrimary ? " is-primary" : ""}" type="button" data-action="friends-flow-action" data-friends-action="${action.target}" aria-label="${action.label}"><i data-lucide="${action.isPrimary ? "check" : "x"}" aria-hidden="true"></i><span>${action.label}</span></button>`).join("");
    return `<div class="friends-flow-actions">${actions}</div>`;
  }

  function renderFriendsList() {
    const list = document.querySelector("#friends-list");
    const hint = document.querySelector("#friends-hint");
    const flow = document.querySelector("#friends-flow");
    const nav = document.querySelector(".friends-flow-nav");
    const back = document.querySelector('.friends-scene .scene-header [data-action]');
    if (list) list.hidden = false;
    if (hint) hint.hidden = false;
    if (flow) flow.hidden = true;
    if (nav) nav.hidden = true;
    if (back) {
      back.dataset.action = "apps";
      back.setAttribute("aria-label", "Back to apps");
      back.title = "Back";
    }
  }

  function openFriendsFlow() {
    const list = document.querySelector("#friends-list");
    const hint = document.querySelector("#friends-hint");
    const flow = document.querySelector("#friends-flow");
    const nav = document.querySelector(".friends-flow-nav");
    ...
    if (back) {
      back.dataset.action = "friends-list";
      back.setAttribute("aria-label", "Back to friends list");
      back.title = "Back to friends";
    }
    renderFriendsFlow(0, { silent: true });
  }

  function renderFriendsFlow(index = state.friendsFlowIndex, { silent = false } = {}) {
    const flow = document.querySelector("#friends-flow");
    if (!flow) return;
    const total = friendsFlowSteps.length;
    const boundedIndex = Math.max(0, Math.min(total - 1, Number(index) || 0));
    const step = friendsFlowSteps[boundedIndex];
    state.friendsFlowIndex = boundedIndex;
    flow.dataset.step = step.id;
    screen.classList.toggle("is-screen-contacting", ["nearby", "partners", "creating"].includes(step.id));
    flow.innerHTML = `<article class="friends-flow-card"><p class="friends-flow-kicker">${step.kicker}</p>${friendsFlowVisualMarkup(step)}<h3>${step.title}</h3><p class="friends-flow-subtitle">${step.subtitle}</p>${friendsFlowActionMarkup(step)}</article>`;
    flow.querySelector(".friends-flow-greeting-video")?.play().catch(() => {});

    const position = document.querySelector("#friends-flow-position");
    const progress = document.querySelector("#friends-flow-progress-bar");
    const pagerButtons = [...document.querySelectorAll(".friends-flow-nav > button")];
    if (position) position.textContent = `${boundedIndex + 1} / ${total}`;
    if (progress) progress.style.width = `${((boundedIndex + 1) / total) * 100}%`;
    pagerButtons[0]?.toggleAttribute("disabled", boundedIndex === 0);
    pagerButtons[1]?.toggleAttribute("disabled", boundedIndex === total - 1);
    lucide.createIcons({ attrs: { "aria-hidden": "true" } });
    if (!silent) announce(`${step.title}. ${step.subtitle}`);
  }

  function runFriendsFlowAction(action) {
    if (!action) return;
    if (action === "make-friends") {
      showToast("More Friends screens are hidden in this preview");
      return;
    }
    if (action === "become-partners") {
      showToast("More Friends screens are hidden in this preview");
      return;
    }
    if (action === "apps") {
    ...
    const targetIndex = friendsFlowSteps.findIndex((step) => step.id === action);
    if (targetIndex >= 0) {
      renderFriendsFlow(targetIndex);
      return;
    }
    showToast("More Friends screens are hidden in this preview");
  }
  ...
  function qrBindVisualMarkup() {
    return `<div class="onboarding-visual"><div class="qr-bind-visual">
      <div class="pair-qr-frame">
        ${qrCodeMarkup()}
        <span class="pair-qr-scanline" aria-hidden="true"></span>
      </div>
  ...
  function wifiNetworksMarkup() {
    const networks = wifiNetworkNames.map((name) => ({ name }));
  ...
  function renderOnboardingStep(index) {
    ...
    document.querySelector("#onboarding-count").textContent = step.id === "language"
      ? "Language"
      : step.id === "wifi-select" || step.id === "wifi-password"
        ? (step.id === "wifi-select" ? "Choose Wi-Fi" : "")
        : step.count;
    const title = isMeet
    ...
    if (step.id === "brand") {
      state.pairTimers.push(window.setTimeout(() => renderOnboardingStep(boundedIndex + 1), 3000));
    }
  ...
  function goTo(sceneName, options = {}) {
    ...
    if (current && !options.fromHistory) {
      state.sceneHistory.push(current.dataset.scene);
      if (state.sceneHistory.length > 20) state.sceneHistory.shift();
    }
    ...
    if (sceneName !== "pairing" && sceneName !== "friends") {
      screen.classList.remove("is-screen-contacting");
    }
  ...
  function goBack() {
    const previousScene = state.sceneHistory.pop();
    if (!previousScene) return false;
    goTo(previousScene, { fromHistory: true });
    return true;
  }
  ...
  function setChatMode(mode) {
    ...
    const character = document.querySelector(".chat-character video, .chat-character img");
    const characterButton = document.querySelector(".chat-character");
    const setCharacterLabel = (value) => {
      if (!character) return;
      if (character.tagName === "VIDEO") character.setAttribute("aria-label", value);
      else character.alt = value;
    };
    ...
    if (mode === "locked") {
      ...
      visual.hidden = true;
      setCharacterLabel(`${getCompanion().name} waiting while the microphone is locked`);
      return;
    }
    ...
    if (mode === "listening") {
      ...
      setCharacterLabel(`${getCompanion().name} listening`);
    } else if (mode === "thinking") {
      label.textContent = "Thinking";
      hint.textContent = "Swipe right to end";
      visual.classList.add("is-thinking");
      setCharacterLabel(`${getCompanion().name} thinking`);
    } else {
      label.textContent = "Speaking";
      hint.textContent = "Tap to interrupt";
      visual.classList.add("is-speaking");
      setCharacterLabel(`${getCompanion().name} speaking`);
    }
  ...
  function toggleCareMode() {
    state.careEnabled = !state.careEnabled;
    const careScene = document.querySelector('[data-scene="care"]');
    const toggle = careScene?.querySelector('[data-action="toggle-care"]');
    toggle?.classList.toggle("is-off", !state.careEnabled);
    toggle?.setAttribute("aria-checked", String(state.careEnabled));
    toggle?.setAttribute("aria-label", `${state.careEnabled ? "Turn off" : "Turn on"} care mode`);
    careScene?.classList.toggle("is-paused", !state.careEnabled);
    showToast(state.careEnabled ? "Care mode on" : "Care mode off");
    announce(state.careEnabled ? "Care mode on" : "Care mode off");
  }
  ...
  function handleSwipe(deltaX, deltaY) {
    ...
    if (horizontal && Math.abs(deltaX) >= threshold) {
      if (state.scene === "friends") {
        renderFriendsFlow(state.friendsFlowIndex + (deltaX < 0 ? 1 : -1));
        return;
      }
      if (deltaX > 0) {
        flashEdge("right");
  ...
  document.addEventListener("click", (event) => {
    ...
    if ((action === "home" || action === "apps") && actionTarget.closest(".scene-header") && goBack()) {
      return;
    }

    if (action === "friends-flow-action") {
      runFriendsFlowAction(event.target.closest("[data-friends-action]")?.dataset.friendsAction);
      return;
    }
    if (action === "friends-open-flow") {
      openFriendsFlow();
      return;
    }
    ...
    if (action === "friends-prev") {
      renderFriendsFlow(state.friendsFlowIndex - 1);
      return;
    }
    if (action === "friends-next") {
      renderFriendsFlow(state.friendsFlowIndex + 1);
      return;
    }

    const destinations = new Set(["overview", "home", "apps", "chat", "controls", "notifications", "voice", "care", "reminder", "friends", "states", "pairing"]);
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (state.scene === "friends") {
      if (event.key === "ArrowLeft") renderFriendsFlow(state.friendsFlowIndex - 1);
      else if (event.key === "ArrowRight") renderFriendsFlow(state.friendsFlowIndex + 1);
      else if (event.key === "Escape") goTo("apps");
      return;
    }
  ...
  let inhaling = true;
  window.setInterval(() => {
    if (state.scene !== "care" || !state.careEnabled) return;
    breathTick -= 1;
    if (breathTick <= 0) {
      inhaling = !inhaling;
      breathTick = 4;
